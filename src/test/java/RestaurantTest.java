...
class RestaurantTest {
    ...
    //>>>>>>>>>>>>>>>>>>>>>>>>>OPEN/CLOSED<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    // >>>>>>>>>>TEST CASES TO FIND WHETHER THE RESTAURANT IS OPEN OR CLOSED<<<<<<<<<<<<<<<<<
    @Test
    ...
    //<<<<<<<<<<<<<<<<TEST CASES FOR IMPLEMENTED METHODS TO CALCULATE TOTAL ORDER VALUE>>>>>>>>>>>>>
    @Test
    public void should_display_total_order_value_given_the_list_of_item_names() {
        ...
        Item item2=restaurant.selectItem("Chocolate brownie");

        itemNames.add(item.getName());
        itemNames.add(item1.getName());
        itemNames.add(item2.getName());

        assertEquals(478,restaurant.totalOrderValue(itemNames));
    }
    @Test
    public void should_display_zero_order_value_when_no_item_is_selected_from_the_menu(){
        assertEquals(0,restaurant.totalOrderValue(itemNames));
